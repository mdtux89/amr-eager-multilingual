...
function loadDataset(dataFileTrain, dataFileValid, nclasses, wordsDataLength, posDataLength, depsDataLength)
  ...
  local trainInput = dp.ListView({dp.DataView('bf', xb), dp.DataView('bf', xc), dp.DataView('bf', xd)})
...
function loadExperiment(opt, dictSizePos, dictSizeDeps, outputSize,  wordsDataLength, posDataLength, depsDataLength)
...
--        i = 1
--        for line in io.lines("resources_es/posembs.txt") do
--          vals = line:csvline()
...
--        end
        ...
        words = nn.Sequential()
        dictSizeWords = 0
        for line in io.lines("resources_es/wordembs.txt") do
            dictSizeWords = dictSizeWords + 1
        end
        ...
        paralltab:add(pos)
        paralltab:add(deps)
        embs = paralltab({inputs[1],inputs[2],inputs[3]})
...
local nPos = 0
for line in io.lines("resources_es/postags.txt") do
    nPos = nPos + 1
end
nPos = nPos + 3
--nPos = 52
...
xp = loadExperiment(opt, nPos, nDeps, 2, 3, 3, 6)
xp:run(dataset)
