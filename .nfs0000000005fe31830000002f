...
then
	...
	cat "$1.tmp" | grep '# ::tok-zh ' | sed 's/^# ::tok-zh //' > "$1.sentences"
...
java -mx6g -cp "$CORENLP/*" edu.stanford.nlp.pipeline.StanfordCoreNLP -props "chinese.properties" -file "$1.sentences" -outputFormat text -replaceExtension --outputDirectory "$workdir"
echo "Done!"
