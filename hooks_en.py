...
wikis = {}
for item in open("resources_en/countries.txt").read().splitlines():
    wikis[item.split(",")[0].strip()] = item.split(",")[1].strip()

countries = wikis.keys()

nationalities = {}
for item in open("resources_en/nationalities.txt").read().splitlines():
    c = ""
...
for item in open("resources_en/nationalities2.txt").read().splitlines():
    if item.split("\t")[0] not in nationalities:
        nationalities[item.split("\t")[1].lower()] = item.split("\t")[0]

states = [item.strip() for item in open("resources_en/states.txt").read().splitlines()]

cities = [item.strip() for item in open("resources_en/cities.txt").read().splitlines()]
