...
then
	...
	cat "$1.tmp" | grep '# ::tok-de ' | sed 's/^# ::tok-de //' > "$1.sentences"
...
java -mx6g -cp "$CORENLP/*" edu.stanford.nlp.pipeline.StanfordCoreNLP -props "german.properties" -file "$1.sentences" -outputFormat text -replaceExtension --outputDirectory "$workdir"
echo "Done!"
