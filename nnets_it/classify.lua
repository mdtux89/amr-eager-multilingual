...
function Classify:action(digits, concepts, pos, deps, constr)
  xa = torch.Tensor(1, digits:size()[1])
  xb = torch.Tensor(1, concepts:size()[1])
...
function Classify:label(digits, concepts, pos, deps, constr)
  xa = torch.Tensor(1, digits:size()[1])
  xb = torch.Tensor(1, concepts:size()[1])
  xc = torch.Tensor(1, pos:size()[1])
  xd = torch.Tensor(1, deps:size()[1])
...
function Classify:reentrancy(concepts, pos, deps)
  xa = torch.Tensor(1, concepts:size()[1])
